...
**Version 1.2.1** · 🌍 Language: [English](#english) | [Русский](#русский)
...
*Note (Linux): to stay within the GitHub Releases 2 GB artifact limit, the Linux-CPU build ships without bundled model weights. On first launch the application offers a one-time runtime download (~480 MB) into `~/.local/share/TensorMedia/models`.*
...
*Примечание (Linux): чтобы уложиться в лимит GitHub Releases (2 ГБ), Linux-CPU сборка поставляется без упакованных весов моделей. При первом запуске приложение предложит одноразовую загрузку (~480 МБ) в `~/.local/share/TensorMedia/models`.*
